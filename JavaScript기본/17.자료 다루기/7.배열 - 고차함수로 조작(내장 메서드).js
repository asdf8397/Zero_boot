...
console.log(langs); // output: ['JS','HTML','CSS']
...
console.log(langs_new); // output: ['JS','HTML','CSS']
...
const new_fil = fil.filter(function(el) {
    ...
    if문으로 조건을 붙이는데 typeof el <=은 JS HTML CSS 0,1,2,3,의 배열을 가리키고
    이것이 el === "number"일때 return el은 number이 되게 한다 */
}); 
console.log(fil); // output: ['JS','HTML','CSS',0,1,2,3]
...
console.log(string); // output: ['JS','HTML','CSS']


/* filter를 간단하게 사용할때 */

const filter_1 = ["JS","HTML","CSS",0,1,2,3];
console.log(filter_1); // output: ['JS','HTML','CSS',0,1,2,3]
...
console.log(filter_2); // output: ['JS','HTML','CSS',0,1,2,3]
const string_2 = filter_2.filter(function(el) {
    if(typeof el === "string") {
        return true
        // return true일때는 string ['JS','HTML','CSS']를 출력하고 false일떄는 출력 안함
    }
});
console.log(string_2); // output: ['JS','HTML','CSS']
...
console.log(strings_10); // output: ['JS','HTML','CSS']
